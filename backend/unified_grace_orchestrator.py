...
async def metrics():  
    ...
    if not PROMETHEUS_AVAILABLE:  
        ...
        return Response(content="# Prometheus not available", media_type="text/plain")  
    from fastapi import Response  
    return Response(content=generate_latest(), media_type=CONTENT_TYPE_LATEST)  
 
from backend.monitoring.observability import track_action_execution, track_contract_verification, track_rollback, track_approval, track_benchmark, track_confidence, track_playbook_success_rate, track_mission, track_job 
...
from backend.observability.health import HealthChecker  
...
async def health_check():  
    ...
    golden_signals = metrics_collector.get_golden_signals()  
ECHO is on.
    return {  
        ...
        "performance": {  
            "requests_per_second": golden_signals.requests_per_second,  
            "error_rate": golden_signals.error_rate,  
            "latency_p95_ms": golden_signals.latency_p95_ms  
        }  
    }  
